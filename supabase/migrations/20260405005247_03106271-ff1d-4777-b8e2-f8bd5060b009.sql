...
AS $$
DECLARE
  modules text[] := ARRAY['dashboard','financeiro','diario_obra','cronograma','relatorios','documentos','usuarios','configuracoes'];
  m text;
BEGIN
  ...
  FOREACH m IN ARRAY modules LOOP
    ...
    ELSIF _role = 'trabalhador' THEN
      ...
      ELSIF m IN ('financeiro', 'usuarios', 'configuracoes') THEN
        INSERT INTO public.permissoes_usuario (user_id, modulo, pode_visualizar, pode_criar, pode_editar, pode_excluir)
